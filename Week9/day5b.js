...
function kMostFrequent(numbers, k) {
    //declare frequency map and arr vars
    //myTable is used to track frequency
    //freqNums will be returned to output the k most frequent ints within the input arr
    let numFreqMap = {};
    let freqNums = [];
    //looping through the arr once to create our map
    for (let i = 0; i < numbers.length; i++) {
        numFreqMap.hasOwnProperty(numbers[i]) ? numFreqMap[numbers[i]]++
            : numFreqMap[numbers[i]] = 1;
    }

    let mostFrequentKey = undefined;
    while (k > 0) {
        let mostFrequent = 0;
        for (let key in numFreqMap) {
            if (numFreqMap[key] > mostFrequent) {
                mostFrequent = numFreqMap[key];
                mostFrequentKey = key;
                // console.log(numFreqMap)
            }
        }
        freqNums.push(mostFrequentKey)
        delete numFreqMap[mostFrequentKey];
        k--;
    }
